...
<div class="mt-4 grid grid-cols-2 lg:grid-cols-4 gap-2.5">
    ...
    <div class="rounded-2xl p-4 {{ $s[3] }}">
        <p class="text-xl">{{ $s[0] }}</p>
        <p class="font-black text-2xl mt-1">{{ $s[2] }}</p>
        <p class="text-[11px] font-bold opacity-70">{{ $s[1] }}</p>
    </div>
    @endforeach
</div>

<div class="mt-2.5 grid grid-cols-2 lg:grid-cols-4 gap-2.5">
    @foreach([
    ...
    <div class="rounded-2xl bg-cream-100 border border-ink-900/10 p-4">
        <p class="text-xl">{{ $s[0] }}</p>
        <p class="font-black text-lg mt-1 leading-snug">{{ $s[2] }}</p>
        <p class="text-[11px] font-bold text-ink-500">{{ $s[1] }}</p>
    </div>
    @endforeach
</div>
...
<div class="mt-2.5 rounded-2xl border border-amber-200 bg-amber-50 p-4">
    ...
    <div class="mt-2 grid gap-2">
        ...
        <div class="flex items-center gap-3 rounded-xl bg-white border border-amber-200/60 px-3 py-2">
            ...
            <a href="{{ route('seller.products.edit', $p) }}" class="shrink-0 text-[12px] font-extrabold px-3.5 py-1.5 rounded-full border border-ink-900/15 hover:bg-ink-900 hover:text-white transition">Restock</a>
        </div>
        @endforeach
    </div>
</div>
