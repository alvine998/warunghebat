...
@section('title', 'Promo & Flash Sale — Warung Hebat')
...
<section class="pt-24 sm:pt-28 pb-14 sm:pb-20 max-w-7xl mx-auto px-4 sm:px-6">
    <a href="{{ route('home') }}#flash-sale" class="inline-flex items-center gap-1.5 min-h-10 text-[13px] font-extrabold text-ink-500 hover:text-ink-900">← Beranda</a>

    <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mt-3 mb-7">
        <div>
            <p class="inline-flex text-[11px] font-extrabold tracking-[0.18em] text-white bg-brand-500 rounded-full px-3.5 py-1.5">⚡ PROMO & FLASH SALE</p>
            <h1 class="font-black tracking-tight text-3xl sm:text-5xl mt-3">Serbu sebelum kehabisan</h1>
            <p class="text-ink-500 font-medium text-[15px] mt-2">
                @if($products->total() > 0)
                    {{ $products->total() }} produk promo dari {{ $storeCount }} warung{{ $userLat !== null && $userLng !== null ? ' • radius '.$radiusLabel.' km, diurut dari yang paling dekat' : ' mitra' }}.
                @else
                    Lagi tidak ada promo yang jalan — cek lagi nanti atau intip warung terdekat dulu.
                @endif
            </p>
        </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2.5 sm:gap-3">
        ...
        <article class="reveal group flex flex-col rounded-[20px] sm:rounded-[24px] bg-white border-2 border-brand-500/30 overflow-hidden hover:shadow-xl hover:shadow-brand-500/20 hover:-translate-y-1 transition-all duration-300" style="--reveal-delay:{{ ($i % 4) * 70 }}ms">
            <a href="{{ route('store.show', $store) }}" class="relative block">
                ...
                @endif
                <span class="absolute top-2 left-2 text-[11px] sm:text-[12px] font-black text-white bg-brand-500 rounded-full px-2.5 py-1 shadow-lg">−{{ $p->discountPercent() }}%</span>
            </a>
            <div class="flex flex-1 flex-col p-2.5 sm:p-4">
                <a href="{{ route('store.show', $store) }}" class="block">
                    ...
                    <p class="mt-1.5 flex items-center gap-1.5">
                        @if($p->stock <= 5)
                            <span class="text-[10px] sm:text-[11px] font-extrabold text-red-700 bg-red-50 border border-red-200 rounded-full px-2 py-0.5 whitespace-nowrap">🔥 Sisa {{ $p->stock }}</span>
                        @else
                        ...
                        @endif
                        @if($p->promo_ends_at)
                            <span class="text-[10px] sm:text-[11px] font-bold text-ink-500 whitespace-nowrap">s/d {{ $p->promo_ends_at->format('d M H:i') }}</span>
                        @endif
                    </p>
                ...
                <div class="mt-auto pt-2.5">
                    ...
                    <form method="POST" action="{{ route('cart.store') }}">
                        ...
                        <button type="submit" class="w-full min-h-11 py-2.5 px-2 rounded-full bg-brand-500 text-white text-[12px] sm:text-[13px] font-extrabold hover:bg-ink-900 active:scale-[.98] transition">+ Keranjang</button>
                    </form>
                    @else
                    <button type="button" data-open-login class="w-full min-h-11 py-2.5 px-2 rounded-full bg-brand-500 text-white text-[12px] sm:text-[13px] font-extrabold hover:bg-ink-900 active:scale-[.98] transition">+ Keranjang</button>
                    @endauth
                </div>
                @endif
            </div>
        ...
        <div class="col-span-full rounded-[24px] bg-white border border-dashed border-ink-900/15 p-10 text-center">
            <p class="text-4xl">⚡</p>
            <p class="font-extrabold text-lg mt-2">Belum ada promo di sekitar sini</p>
            ...
            <div class="mt-4 flex flex-wrap justify-center gap-2">
                ...
                <a href="{{ route('home') }}#kategori" class="text-[13px] font-extrabold border-2 border-ink-900/10 px-5 py-2.5 rounded-full">Jelajah kategori</a>
            </div>
        </div>
        @endforelse
    </div>
